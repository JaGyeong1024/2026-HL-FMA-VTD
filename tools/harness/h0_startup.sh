...
ROUTE_CSV=/nonexistent.csv timeout 20 ./start_autonomous.sh mock > "$OUT/csv_missing.log" 2>&1; rc=$?
...
ENGAGE=false ROUTE_CSV="$HOME/hlfma/route/route_pretest_2.csv" ./start_autonomous.sh mock > "$OUT/no_mock.log" 2>&1 < /dev/null & echo $! > "$OUT/p2.pid"
sleep 25
...
sleep 1
ENGAGE=false ROUTE_CSV="$HOME/hlfma/route/route_pretest_2.csv" ./start_autonomous.sh mock > "$OUT/first.log" 2>&1 < /dev/null & echo $! > "$OUT/p3.pid"
sleep 12
ENGAGE=false ROUTE_CSV="$HOME/hlfma/route/route_pretest_2.csv" timeout 20 ./start_autonomous.sh mock > "$OUT/second.log" 2>&1; rc=$?
