...
class VtdAutowareBridge(Node):
    def __init__(self):
        ...
        #   9/12 재측정: 20 km/h 로는 부족. 정지선 12.8 m 앞에서 적색이 들어왔을 때 신호등 모듈의 정지
        #   필요거리 10.3 m > 남은 8.0 m 라 통과 가능으로 판정해 적색에 정지선을 넘었다(0912_062716 t=662s).
        #   12 km/h 면 필요거리 4.8 m 로 여유. 내리막 가속은 원인이 아니다(VTD 는 명령 순가속만 적용, 실측 k=0.08).
        dp('tl_unknown_cap_mps', 3.33)     # [m/s] 미할당 신호 정지선 접근 상한 (12 km/h)
